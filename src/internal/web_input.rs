...
extern "C" {
    ...
    #[wasm_bindgen(js_name = positionMobileKeyboard)]
    fn position_mobile_keyboard_js(x: f64, y: f64, width: f64, height: f64);
...
#[cfg(target_arch = "wasm32")]
pub(crate) fn position_mobile_keyboard(bounds: iced_widget::core::Rectangle) {
    position_mobile_keyboard_js(
        f64::from(bounds.x),
        f64::from(bounds.y),
        f64::from(bounds.width),
        f64::from(bounds.height),
...
#[cfg(not(target_arch = "wasm32"))]
pub(crate) fn position_mobile_keyboard(_bounds: iced_widget::core::Rectangle) {}
