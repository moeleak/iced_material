    ...
    Renderer: text::Renderer,
{
    ...
    pub fn style(mut self, style: impl Fn(&Theme, Status) -> pick_list::Style + 'a) -> Self
    where
        <Theme as pick_list::Catalog>::Class<'a>: From<pick_list::StyleFn<'a, Theme>>,
    {
        self.class = (Box::new(style) as pick_list::StyleFn<'a, Theme>).into();
        self
    }
